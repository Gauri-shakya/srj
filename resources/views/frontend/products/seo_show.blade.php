...
@section('title', $dynamicTitle . ' | ' . App\Models\Setting::get('site_name', 'SRJ Heat Exchangers'))
        ...
        <div class="flex flex-col items-start gap-3">
            ...
            <div class="flex items-center flex-wrap gap-2 md:gap-3 text-xs md:text-sm font-bold uppercase tracking-widest text-white drop-shadow-md mt-2">
                ...
                @if($product->category)
                    ...
                    <i class="fas fa-arrow-right text-red-500 text-[10px] md:text-xs"></i>
                @endif
                <span class="hover:text-red-400 transition-colors border-b border-white pb-0.5 whitespace-nowrap cursor-default">{{ $location->name }}</span>
            </div>
        </div>
    </div>
</div>
